...
async function main() {
  try {
    ...
    console.log('Starting WooCommerce sync to fix categories...');
    
    // Cache categories to avoid DB lookups
    ...
    while (true) {
      ...
      for (const item of products) {
        ...
        if (item.categories && item.categories.length > 0) {
          ...
          // Default to WooCommerce category
        ...
        await prisma.$executeRawUnsafe(`
          ...
          ON CONFLICT ("sku") DO UPDATE SET
            ...
            "stock" = $7, "description" = $8, "images" = $9, "categoryId" = $10, "updatedAt" = NOW()
        `, 
        'prod_wc_' + item.id, name, prodSlug, sku, price, comparePrice, stock,
        description, imagesJson, categoryId, sourceProductId);
      ...
      // Delay to avoid connection pool timeouts
