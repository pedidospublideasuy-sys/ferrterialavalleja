...
async function main() {
  try {
    ...
    console.log('Starting WooCommerce clean sync with Prisma Upsert...');
    ...
    while (true) {
      ...
      // Filter out products with no images, JUST LIKE THE ORIGINAL PANEL SCRIPT DID
      const products = productsData.filter(item => item.images && item.images.length > 0);
      ...
      for (const item of products) {
        ...
        let stock = 100; // default positive stock
        if (item.stock_status === 'outofstock') {
            stock = 0;
        } else if (item.manage_stock && item.stock_quantity !== null) {
        ...
        try {
            ...
            console.error('Error upserting product', sku, e.message);
        }
      }

      console.log('Page ' + page + ' synced! (' + products.length + ' items with images)');
      if (productsData.length < 100) break;
      page++;
