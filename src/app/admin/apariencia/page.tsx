...
export default function AparienciaPage() {
  ...
  const [productTitleBg, setProductTitleBg] = useState('#2a2a2a');
  const [productTitleColor, setProductTitleColor] = useState('#ffffff');
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (s.home_products_title_bg) setProductTitleBg(s.home_products_title_bg);
      if (s.home_products_title_color) setProductTitleColor(s.home_products_title_color);
      if (s.hero_slides) {
  ...
  const saveColores = () => saveSettings({
    ...
    home_products_title_bg: productTitleBg,
    home_products_title_color: productTitleColor,
  }, 'Colores guardados');
      ...
      {tab === 'colores' && (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              <h2 className="font-bold text-sm text-gray-800 mb-1">Títulos de categorías de productos</h2>
              <p className="text-xs text-gray-400 mb-4">Color de fondo y texto de los títulos de cada carrusel del home.</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className="block text-xs font-medium text-gray-600 mb-1">Fondo</label><input type="color" value={productTitleBg} onChange={e => setProductTitleBg(e.target.value)} className="h-10 w-full rounded border cursor-pointer" /></div>
                <div><label className="block text-xs font-medium text-gray-600 mb-1">Texto</label><input type="color" value={productTitleColor} onChange={e => setProductTitleColor(e.target.value)} className="h-10 w-full rounded border cursor-pointer" /></div>
              </div>
            </div>
          </div>
